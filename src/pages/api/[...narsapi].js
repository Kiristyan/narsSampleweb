...
async function handleSignin(req, res) {
  ...
  try {
    ...
    const placeholders = searchTerms.map(() => '(name LIKE ? OR category LIKE ?)').join(' AND ');
    
    const sql = `
      SELECT *, stock_quantity 
      FROM products 
      WHERE deleted = 0 AND (${placeholders})
    `;
...
async function handleGetProducts(req, res) {
  try {
    const products = await query("SELECT *, stock_quantity FROM products WHERE deleted = 0");
...
async function handleGetProductsByCategory(req, res, category) {
  try {
    ...
    const products = await query("SELECT *, stock_quantity FROM products WHERE category = ? AND deleted = 0", [decodedCategory]);
...
async function handleGetLimitedItems(req, res) {
  try {
    ...
    const limitedItems = await query("SELECT *, stock_quantity FROM products WHERE category = 'Limited Items' AND deleted = 0");
